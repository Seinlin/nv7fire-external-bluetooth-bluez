...
static void device_event(int event, int index)
{
	...
	case HCI_DEV_DOWN:
		...
		/*
    case HCI_DEV_RESUME:
        info("HCI dev %d reset", index);
		manager_reset_adapter(index);
		//stop_security_manager(index);
		break;
		*/
    case HCI_DEV_RUN:
        info("HCI dev %d communication", index);
        manager_run_adapter(index);
        break;
    case HCI_DEV_IDLE:
        info("HCI dev %d idle", index);
        manager_idle_adapter(index);
        break;
	}
}
